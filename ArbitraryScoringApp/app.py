...
def playerScoring(pts, func, fts = True):
	...
	for player,games in pts.iteritems():
		r.append((player,round(mean([sum(map( lambda x: func(x) , i['shot_distance']))+ (i['fts'] if fts else 0) for i in games ]),1), round(mean([sum( i['points'] )+ (i['fts'] if fts else 0) for i in games ]),1), round(1.*sum(reduce(lambda x,y: x+y, [ map( lambda x: func(x) , i['shot_distance'])  + [(i['fts'] if fts else 0)]  for i in games ]))/sum([i['shots'] for i in games] ),2), round(1.*sum(reduce(lambda x,y: x+y, [ i['points'] + [(i['fts'] if fts else 0)]  for i in games ]))/sum([i['shots'] for i in games] ),2)
		))
